...
public class C_Cliente 
{
    public Object[][] cargarTabla() 
    {
        ...
        Object[][] caract = new Object[Clientes.size()/7][7];
        for(int j = 0; j < Clientes.size()/7;j++)
        {
            for(int i =0; i < 7;i++)
            {
                caract[j][i] = Clientes.get(i+(j*7));
            }
        }
        return caract;      
    }
    ...
    public boolean Modificar(int id_cliente, String nombre, String apellidos, String direccion, String nacimiento, String telefono, String email)
    {
        Conexion_Controller con = new Conexion_Controller();
        con.crearConexion();
        ...
        try
        {
            ...
            moda.executeUpdate("update TB_Cliente set Nombre = '"+nombre+"', apellidos = '"+apellidos+"', direccion = '"+direccion+"', nacimiento = '"+nacimiento+"', telefono = '"+telefono+"', email = '"+email+"'  where Codigo_Autor = "+id_cliente+";");
        }
    ...
    public Object[][] cargarTablaIdCliente(int id_cliente) 
    {
        ...
        Object[][] caract = new Object[Cliente.size()/7][7];
        for(int j =0; j < Cliente.size()/7;j++)
        {
            for(int i =0; i < 7;i++)
            {
                caract[j][i] = Cliente.get(i+(j*7));
            }
        }
        return caract;      
    }
